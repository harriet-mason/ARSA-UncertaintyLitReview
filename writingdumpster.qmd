...
- @[intervaldata] Propse two visualisation methods, segment and dandelion plots that offer more information than the existing methods as a new way to conduct EDA with interval variables (“propose two new plots for EDIA”). They specify it's need as you can only see "correlation in center" and not "correlation in range". Seems like a signal vs noise problem. Variance (in this case range) is a different variable in this case. Unclear why you wouldn't just do a scatter plot of the ranges.
- @[visualmirage] Have several elements that they consider to cause "visual mirages", from improper imputation of missing values, to summary functions obfuscating the variabiliy of the data. It seems to be a bizzare mismatch of different types of information that are grouped together with "visual mirage".
...
- [@unwinhdr] “One persons statistics might be another person’s raw data”
- [@deepuncert] Geoscience review of uncertainty visualisations - discussing problems and how to move forward
 - @[ieeetrust]  Suggests that a plot has uncertainty if it "may hinder the user in gaining insights from the data" is bizzare. Specific cases are resolution and clutter. All forms of communication are finite in what they can convey, this is not a useful definition and will only cause confusion.
