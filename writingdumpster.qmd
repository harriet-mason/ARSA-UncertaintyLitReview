...
## CITATIONS WHERE SECTION WAS REMOVED
- Datasets such as Anscombe's quartet [@anscombe] or the Datasaurus Dozen [@datasaurus; @datasaurpkg] show a case where visual statistics highlight elements of the data that are invisible to the typical summary statistics. 
...
# Taken from ggdibbler dumpster
- We will not go through how this uncertainty is quantified, but we will rely on the fact that all uncertainty, no matter how abstract, can always be quantified by a random variable [@Mason2024].   
  - Make sure I actually go back and make this argument.
...
#### Distributions are both necessary and sufficient for uncertainty visualisation

### Bayesian vs Frequentist sample?
###  Dists of Dists (Check if it can be moved to dist section of paper)
...
# Necessary changes for the following papers
- Make the inference point clearer. Specifically mention that the only way for EDA, a process with no specific inferential statistic, to *have* uncertainty, a by product of inference, is if the input itself is uncertain (i.e. inference has already occured before we started graphing).
- Establish difference between EDA and Confirmatory (deductive vs inductive, searing for a hypothesis vs validating a hypothesis).
- Make the argument that the only way to quantify uncertainty is as a random variable (currently in ggdibbler, move to annual reviews)
...
- Only give a broad idea of the evaluation problems, move most of the examples of contradicting results to the evaluation paper
...
- @[Kay2019] "It is important to have clear, domain- and task-relevant guidance on how to determine how much value to suppress—or if to suppress value at all—due to uncertainty". Signal vs noise issue.
...
- [@cookhdr] State that “EDA starts from the data”
...
- (problem with inference focused uncertainty) @Hofman2020 commented that "theoretically" the sampling distribution of the mean and the prediction interval of a new observation are equal "so long as one knows the sample size", but does not seem to provide participants with that sample size, or recognise the assumptions and background knowledge that would be required to compare the two.
...
- @Kale2018 did actually ask indirect questions and evaluate uncertainty as noise but every single other thing about the experiment was so stupid I cannot justify talking about it.
