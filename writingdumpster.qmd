...
# Citation Gaps
Cite Gap 1: When did uncertainty visualisaiton become its own subfield of visualisation?

Cite Gap 2: When was the first "uncertainty visualisation" used (likely a box plot of histogram)

Cite Gap 3: Some consider it to be synonymous with specific terms defined in mathematics, such as probability [*HERE*], variance[*HERE*], error[*HERE*], or precision[*HERE*]. Others consider it to be an encompassing umbrella term of which for these mathematically defined objects are only examples[*HERE*], and include concepts that are somewhat related, such as missing values[*HERE*]

Cite Gap 3.5: This wide array of uncertainty definitions becomes a snake eating its own tail, as many authors opt to redefine uncertainty themselves[*HERE*], ignore defining uncertainty entirely [*HERE*], or pick a definition from a seemingly randomly previously published papers [*HERE*], all of which make variance in uncertainty definitions even worse

Cite Gap 4:The uncertainty visualisation literature is completely awash with papers that define uncertainty using a vague encompassing phrase but go on to *quantify* uncertainty as a PDF, error, or some other easily quantified mathematical object when it comes time to do the visualisation.

Cite Gap 5: This definition swap happens so subtly that it seems to go unnoticed by authors writing the papers, and the fact that "uncertainty visualisation" methods dont even notice they are exclusively focus on depictions of easily quantifiable uncertainty, such as error [*HERE*], mass [*HERE*], or variance [*HERE*]

Cite Gap 6: This has left the expressions of uncertainty that are hard to quantify or visually differentiate untouched, despite many papers calling for their invention [*HERE*]

Cite Gap 7: Taxonomies split uncertainty based on an endless stream of arbitrary lines in the sand, such as whether the uncertainty is due to true randomness or a lack of knowledge [@Spiegelhalter2017 *HERE*], if the uncertainty is in the the attribute, spatial elements, or temporal element of the data [*HERE*], whether the uncertainty is scientific (e.g. error) or human (e.g. disagreement among parties) [*HERE*], if the uncertainty is random or systematic [*HERE*], statistical or bounded [], accuracy or precision [*HERE*], if the uncertainty is about the past or the future [*HERE*], the stage of the data analysis pipeline it comes from [*HERE*], the severity of the uncertainty [*HERE*], how quantifiable the uncertainty is [@Spiegelhalter2017], and many, many, many more examples.

Cite Gap 8: It is commonly noted that each uncertainty must be discussed in isolation, but combining the uncertainty from every stage is near impossible [@Spiegelhalter2017 *MORE HERE*]

Cite Gap 9: Many papers will boast a title that claims to be about uncertainty visualisaiton, but simply depicts different visual representations of a PDF, however it takes reading the methodology to find this out

Cite Gap 9: For example @Leland2005 mentions that popular graphics, such as pie charts and bar charts omit uncertainty, however at least one or both of these charts are used in most "uncertainty visualisation" experiments [@Ibrekk1987 *MORE HERE*]

Cite Gap 10: Uncertainty Visualisation Taxonomies

Cite Gap 11: Papers will often discuss uncertainty in relation to one of these motivating reasons, the evaluation experiments motivated by (1) often perform different visualisations of PDFs, while the papers motivated by (2) will focus on trying to impute uncertainty as error within one of the existing channels, such as colour

Cite Gap 12: Visualisation authors are almost unanimous in commenting that the "information" in two plots must be the same in order for the visual techniques to be compared

Cite Gap 13: Examples of what is "information" in a visualisation

Cite Gap 14: Examples of mathematically obvious results

Cite Gap 15: Examples of replicated perceptual task experiments

Cite Gap 16: Examples of value extraction decision making tasks

Cite Gap 17: Examples of studies where the correct choice was to *ignore* the uncertainty information

Cite Gap 18: Examples of studies where authors measure trust

## Extra Section Cite Gaps
Cite Gap - New Section: Logical fallacy interpretation of mathematically unanswerable questions. If I tell you my cousins age, and then ask you to guess my age, it would not be unreasonable to guess the age I provided for my cousin. Citing this work to state "people incorrectly believe my cousins age is my age" would
...
# Moved over notes
- These industry visualisation are likely made with a large amount of back and forth between different levels of an organisation, something that is important for good visual communication [@Spiegelhalter2017]. 
- @Boone2018 organised responses from participants by the approach they used and tried to account for participants prior beliefs (the method of eliciting prior beleifs was, however, questionable). 
- @Correll2018 “place your 5 ships on the safest locations on the board.” (minimise danger)
...
- @Blenkinsop2000 asked different questions for each uncertainty visualisation, indicating they had some awareness that the question has a significant impact on whether or not the visualisation should be used.
...
- Some markers of probability that have common uses, probabilities such as 0 (impossible), 0.5 (a fair coin flip), 1 (certain) are easy for people to have an internal sense of *hullman paper*. 
...
- @Munzner2009, in her nested model for visualisation, comments on how even something as low level as poor problem abstraction can cause down stream effects and become impossible to ignore. 
- @Otsuka2023 highlights that the choices made in the ontological process of boiling real world events down into statistical objects can influence later stages of analysis due to what was considred signal and what was considered noise. 
- @Meng2021 stresses the importance of treating data as an input from another process with noise and variance, rather than as a neutral input.
...
- @Griethe2006 found that there there are two ways to visualise uncertianty, implicitly and explicitly. When a visualisaion is implicit, the uncertainty is considered an uninteresting feature of the data, and instead of mapping some quantification of uncertainty to a visual feature, data is filtered out if it does not meet a specified "certainty" threshold. These statements clearly points towards a signal/noise dichotomy with normal visualisation and uncertainty visualisation.
...
- @Spiegelhalter2017 common sentiments such as avoiding chart junk, considering a table in place of a visualsiaiton, use of narrative labels, managing emotion, less is more approach, etc, are the main concepts expressed. - @Spiegelhalter2017 suggests that the graphic should allow "part-to-whole" comparison on an appropriate scale, which seems to be an extension of the framing concept discussed when speaking about uncertainty communication more broadly.
...
- @Hullman2016 discussed the role of heuristics in the analysis of results and suggested authors look for signs of them in responses and ask subjects to describe their strategy in their response. 
...
@Daradkeh2015 found that people tended to ignore or give little weight probability information and instead focused on the severity of undersierable outcomes.
- @wu2023rational attempted to remedy this issue caused by designing a "rational agent benchmark" such that participants in an experiment are compared to a rational agent. This suggestion is an insufficient band-aid solution that is built upon the same invalid assumptions as the rest of the literature, and simply digs the field deeper into these problems, rather than solve them. 
- @wu2023rational almost seems to recognise the problem of decision making experiments with payment schemes or correct/incorrect answers, but instead of understanding this boils these tasks down to value extraction, they suggest participants should be compared to a "rational agent". This rational agent acts as a benchmark, in the same way a ground truth statistic does for value extraction experiments, except the authors don't seem to realise that is what they have done. This paper highlights many decision making tasks are simply a value extraction experiment without realising it, this paper included.
- make sure quantified vs discussed uncertainty is mentioned
...
- we consider a summary table as a visualisation, illuminate graphics with words and numbers, design graphics to allow a part-to-whole comparison on an appropriate scale, use images and metaphors to gain/retain attention but dont arouse undue emotion, assume low numeracy of a general public audience and adopt a less-is-more approach by reducing the need for inference and making clear and explicit comparison, be cautious about interactivity and animations (may introduce unnecessary complexity), avoid chart junk (like 3d bar charts) @Spiegelhalter2017
- @Maceachren2012 found to be the worst aesthetics to map to uncertainty to as they don't have an intuitive interpretation.
...
Using the most commonly presented methods In the absense of an encapsulating mathematical definition this will not change. That being said, even with a definition that allows us to combine these uncertainties into a single value, it would still be beneficial for authors to develop specialty visualisations for different sources of uncertainty, so analysts can utilise visual tools to understand the uncertainty in their data. still might be desirable to see which stage of the analysis is creating uncertainty in the visualisaiton.
...
- This is likely because The idea that uncertainty can only be defined in the presence of a motivating question is well grounded in most areas of statistics. The entire process of data analysis, from deciding what should be observed as data through to communicating that data in a plot is governed by human decision and the goal of an analysis.
- We know uncertainty cannot be entirely explained by individual concepts that *have* been defined, such as probability, variance, error, missing values, precision, or related concepts that pop up throughout the data analysis pipeline. We also know it likely has some relationship with the *task* we are considering, given that many authors have attributed the noise in uncertainty visualisation research as a type of "task" dependence. Additionally, it should be clear that we wouldn't consider uncertainty to be an appropriate term for *everything* in statistics would we?
- What exactly is uncertainty, then? Let us consider the concepts related to uncertainty that have been discussed so far. We know uncertainty cannot be entirely explained by individual concepts that *have* been defined, such as probability, variance, error, missing values, precision, or related concepts that pop up throughout the data analysis pipeline. We also know it likely has some relationship with the *task* we are considering, given that many authors have attributed the noise in uncertainty visualisation research as a type of "task" dependence. Additionally, it should be clear that we wouldn't consider uncertainty to be an appropriate term for *everything* in statistics. For example, I wouldn't describe the population parameters of a distribution I have simulated to be "uncertain" 
