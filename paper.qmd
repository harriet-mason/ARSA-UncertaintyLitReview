...
::: {#fig-forest layout="[[59,-2, 39], [59,-2, 39]]" layout-valign="bottom"}

![Data analysis Goal](forestwalk/fig1.jpg){#fig-forest1}

![The typical modelling process](forestwalk/fig2.jpg){#fig-forest2}

![Making zero assumption at every stage](forestwalk/fig3.jpg){#fig-forest3}

![An example of "scenario uncertainty" with a confidence interval](forestwalk/fig4.jpg){#fig-forest4}
...
::: aside
This section is supposed to be an explanation of the general literature, but the definition problem makes it hard to sort the general literature. While there is that overview from spiegelhatter, it is mostly a discussion on risk communication, communicating ambiguous information is not discussed much. The focus on risk (uncertainty as signal) and not uncertainty (uncertainty as noise) is an issue, but the bigger problem is caused by the vagueness of uncertainty issue. Since a paper will be titled "uncertainty communication" it could be any number of things mathematically, but 90% of the time is it just communicating risks or uncertainty as signal (and very rarely uncertainty as noise). I can only tell which one a paper is *once I am in the method section* and even then it is sometimes not explained. Also, since I haven't been using this metric since the beginning I need to go back and check this for all the papers. I also think this section would benefit from the table, I can quantify how many papers communicate uncertaitny as signal, consider risk-aversion and truth but dont mention transparency, etc. I think the only way to back up a lot of my general comments is with these tables that show how widespread these issues are. This process is tedious but because definitions for key words are redefined within every paper, I dont think a clustering method will help
:::
...
::: aside
Again, this is why I had that table, because these papers are impossible to organise without some metrics. Things that are highly important for uncertainty as I understand it, are not mentioned at all or only mentioned knee deep in the method, so reading and organising these papers such that the information can be combined is a bit of a nightmare. If I *dont* find this information uncertainty, I will end up with the same problem as *all* the other uncertainty visualisation literature reviews, which is that the information is *all conflicting and it is impossible to identify any results*. The other literature reviews *clearly state this*.
:::

There are a wealth of uncertainty communication and visualisation literature reviews that already exist. 

@Kinkeldey2014 summarised the literature specifically for geopsatial uncertainty visualisations and attempted to describe similarities in their characteristics. They found large difficulties in the ability
...
include appropriate narrative labels (such as magnitudes through tick marks), 
...
These literature reviews highlight a common theme,
In contrast to these literature reviews, we do not have a specific domain or tool analysis we intend to perform. Rather, the point of this literature review is to understand the *purpose* of visualising uncertainty (i.e. signal supression), and understand if the current visualisation tools are sufficient to achieve that purpose and if not, where there is need for improvement. Additionally, rather than discussing "uncertainty visualisation" as its own field, we view it as a subfield of both uncertainty and visualisation, and understand if this change in conceptualisation can alleviate or help us understand some of the issues faced by previous authors attempting to summarise the uncertainty visualisation literature.
