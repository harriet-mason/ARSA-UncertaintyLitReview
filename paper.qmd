...
There are several issues that become apparent when looking at this list. First of all, 
Looking at these taxonomies there are three key features of interest that become apparent. People seem to be interested in the:

1) What stage of the analyis or what group is causing the uncertainty (the location)
2) How well the uncertainty can be quantified (the level), and
3) Whether or not the uncertainty is believed to be caused by natural variance or a lack of information (Nature)

 . Additionally, since many papers will go on to define uncertainty as a probability or a density in their method, the actual organisation of "uncertainty" is irrelevant to their results since a probability distribution is 

or where it exists in the broader scope of uncertainty communication. If a taxonomy is presented it is rarely considered when discussing differences in calculating, conceptualising,  The definition is also rarely dicussed beyond the introudction of a paper. to its place in the broader scope of of these definitions are presented with little justification as to why 


Type A evaluation of standard uncertainty, and the other which is based on scientific judgement (such as experience and specifications), called the Type B evaluation of standard uncertainty.





It can be seen that while these taxonomies are both attempting to organise similar aspects of uncertainty, they have little overlap. The reason for this is relatively simple. Uncertainty is often only defined in so far as it is needed. 


Ultimately these taxonomies end up alienating those that actually work with uncertainty by being too narrow. Uncertainty is a broad and complicated topic and while oversimplified taxonomies might help us hold the concept of uncertainty in our head, they do little to help with the practical question of how to quantify, visualise, or communicate uncertainty.


- mention that utypo taxonomy contains all these taxonomies and can be easily built upon without making a new taxonomy.
...
This way of conceptualising and organising uncertainty is not a fringe idea and is backed up by several related ideas in statistics. 
- also makes it easier for people to communicate exactly what they need visualisation tools for. Look at geography papers and how their specific uncertainty considerations have been ignored for years.
- can organise other "uncertainties" according to this framework
- it also gives us rules for combining uncertainty since statistical uncertainty can be combined with staitstical uncertainty, and outcome uncertainty can be combined with outcome uncertainty.
Additionally, this definition of uncertainty is helpful in understanding *how* uncertainty should be communicated and the consideration we should make.
...
- VSUP
- the map interactive map that susuan showed me
- the pixel map
- The new york times wealthy people that end up in different economics classes, white vs black people
- the animated surface classifications from that satelite images experimental paper
- the climate change scenario uncertainty
