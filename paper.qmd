...
Risk and uncertainty are quite different in practice. Uncertainty information is typically included in some form of statistical communication to offer some transparency to the viewer. Data analysis is a long and complicated process, so much so that even experts in the field can find themselves lost with a single incorrect assumption, so uncertainty can help pull back the curtain on this process and prevent people from understanding it as a human agnostic, entirely deterministic process. When science authors discuss "risk avoidance" in the uncertainty communication conversations, it holds with it some implication that heuristics such as risk avoidance are relevant in every conversation about uncertainty communication, however that is not true.

There are some elements of risk communication that seem to translate to uncertaitny communication more broadly. Using words to communicate risk (as well as uncertainty more broadly) is discouraged because people can misinterpret the actual risk involved, so when possible numerical estimates of risk should be communicated, however this can make information harder to understand for those with poor numeracy skills [@Spiegelhalter2017].


likely due uncertainty and risk being near synonymous, it is difficult to read a paper that discusses uncertainty that does not make a nod towards human attitudes towards risk. Even when the uncertainty being discussed in that particular paper has very little to do with risk.

Risk-aversion is the tendancy of people to prefer outcomes of low uncertainty to those outcomes with high uncertainty despute the outcomes with high uncertainty having a higher or equal expected outcome. The story of uncertainty communication is littered with conflated ideas, and that issue is highly visible here. Despite being mentioned in a large proportion of papers that discuss uncertainty risk aversion, seems to have very little to do communicating uncertainty. As a matter of fact, it has so little to do with uncertainty communication that we would not even mention it in this paper were it not for the fact that ignoring it would suggest we had missed an important section of the literature.


 Additionally, risk avoidance is observed when people have a choice between a *deterministic* outcome and a *risky* event that has higher expected value. How this psychological heuristic is therefore related to all uncertainty visualisation despite uncertainty communication being different in both the scenario (i.e. it is highly unlikely that we need to communicate a choice between a perfectly deterministic and an uncertain event) and in the type of uncertainty (since risk and uncertainty are not synonyms).

The regular conflation of risk and uncertainty despite clear and specific differences, without any explanation of how these terms are related, implies that a large portion of uncertainty literature is simply reporting information from any research that is uncertainty adjacent without considerations for the specifics of how these concepts would be related to uncertainty.

It is not even clear that this behaviour is entirely irrational either, considering risk aversion is, at its core, a trade off between bias and variance. If risk-aversion is truly "irrational" behaviour, every statistical textbook that discusses the use of a biased but consistent estimator in a fite sample case (a common example would be specific cases of the maximum likelihood estimator) should immediately toss those chapters out and the authors should feel embarassed for that *clearly* irrational line of thinking. Of course I am making a joke but this does highlight a clear problem in the way risk-aversion is discussed in the literature. Discussing risk-aversion as though it is some diversion from a perfectly rational, mathematical choice, implies there is *no value to a decrease in variance* which is not even true *within mathematics*. The concept of risk aversion treats risk as "noise", as though it is something that only exists to be ignored, but if the common place use of biased MLE's is an indication of anything, it is that *ignoring uncertainty* is the irrational choice.

Selecting a biased is technically statisticians doing "risk aversion", but when used to ironically when it is used to select models it is not viewed as an irrational human fault. 

This issue, where psychological considerations seem to be entirely devorced from what is *actually* being discussed extends to the conversation around trust as well. The idea of measuring or being concerned about audiences perception of trust, without first establishing if what we are communicating is *trustworthy*, . The goal of uncertainty visualisation should not be to be trustworthy, but rather to be transparent (which can facilitate trust).
...
Uncertainty visualisation is related to uncertainty communication in the same way that visualisation more broadly is related communication more broadly. Visualisation is fast, it can communicate more complicated ideas that what can be done with text alone, and 
