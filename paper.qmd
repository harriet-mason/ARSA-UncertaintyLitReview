...
### A navigating a forest analogy
When communicating uncertainty information, it is important to ask if we should consider the bias to be variance. We often talk about the "bias variance trade off" but we rarely discuss bias as "cashed in" variance, which in some sense, it is. If you are struggling to understand how bias is just traded off variance, let us walk through an analogy. 
...
This analogy may seem silly but it is explaining a very real problem that exists in uncertainty communication. The bias and variance trade off is commonly discussed in modelling, but rarely mentioned when we discuss communicating uncertainty. The reality is that every point in our data analysis, is full of decisions. Every time we make a decision at some point in this pipeline, we are impacting this bias and variance trade off. We have swapped out the variance from a large range of choices for the bias that may arise from an incorrect decision. Validation methods (such as cross validation, assumption checks, etc) help to guide the choices we make in the data analysis pipeline, but they can never offer any kind of certainty or truth considering validation methods themselves are also only estimates. When we consider the "uncertainty" of a model we usually only consider the variance of the final stage, the prediction error for a specific model and a specific set of features, and ignore the variance (that has been converted into bias by our selections) that was introduced at earlier stages (e.g. such as in data collection, when cleaning the data, or when considering alternative models). This is *well known* in modelling, it is why big tech companies try to collect as much data as they can and then put it in a neural network and hope they have enough data and enough feedback to avoid the this bias at *all* stages of the analysis. Recording *everything* means we don't even have to put any poles down at the data collection stage. Sometimes these statements are couched in a single assumption, such as when a time series model's confidence interval is constructed "assuming previous trends will continue into the future". This trade off issue means the uncertainty we typically estimate, in the form of a confidence inteval or traces or whatever you choose, has confidence intervals that are too small for what you actually consider the goal of the analysis to be. Unfortunately factors that typically decrease the variance, such as sample size, can only have an effect at the levels where that information is used to inform the choices. So confidence intervals that are incredibly small but built on a biased sample will be misleading and will be too small to *actually* predict future data. It is not uncommon for true results or models produced by a different team to sit entirely outside of the confidence band of another. This might be obvious when it is read, but the conversation about whether or not we should consider what was previously variance, to be variance when we communicate our analysis, is something that is rarely discussed although it is important. 

Therefore, we must ask ourselves, *why* are we visualsing uncertainty? Uncertainty is only thought of as the final stages because to give a point prediction for a random process feels silly. By hiding the selections we made at earlier stages, when we traded that variance in for bias, we functionally made *point predictions* but it was just invisible to us.
