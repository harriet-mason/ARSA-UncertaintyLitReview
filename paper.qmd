...
@fig-data shows the first six rows of our example data, along with the geographical boundaries of our data set.  The temperature variable was generated using a quadratic spatial trend, that is $Temperature = 28-SLongitude^2 + SLatitude^2$ where the $SLongitide$ and $SLatitude$ are the scaled (to standard normal) longitude and latitude of the county's centroid. The high and low cases for the sampling variance are randomly sampled from a uniform distribution. The low standard errors are drawn from a $U_{[0,1]}$ distribution, while the high standard error cases are drawn from a a $U_{[1,2]}$ distribution. This is the data we will be using in our spatial uncertaity examples for the rest of the paper.
