...
show_pal1 <- function (colours, borders = NULL, cex_label = 1, 
    ncol = NULL) {
  ...
  text(c(0,col(colours)), -c(1,row(colours))-0.15, 21:29, 
              cex = 1, col = "black")
  text(4, -1.55, "Temperature",cex = 1.5, col = "black")
}

p1a
p1b
show_pal1(basecols, ncol=8, borders=NA)
...
## Decision making does not need uncertainty visualisation
Of course, uncertainty visualisation is not only motivated by signal-suppression. Some authors claim the purpose of uncertainty is not transparency, but rather important information for effective decision making [@Ibrekk1987; @uncertchap2022; @Hullman2016; @Cheong2016; @Boone2018; @Padilla2017]. While uncertainty can provide useful information in decision making, that information is actually signal that is required due to incomplete information. This is easiest to understand with an example.

Imagine you are trying to decide if you want to bring an umbrella with you to work. An umbrella is annoying to bring and you only want to bring it if there is a 10% chance of rain (or higher). Unfortunately, your weather prediction app only provides you with the predicted daily rainfall. Therefore, your decision will be improved with the inclusion of uncertainty, *not* because uncertainty is important for your decision, but because it gives you the tools required to calculate the *actual* statistic you are basing your decision on.

While my decision will be improved with the inclusion of uncertainty information, that is not because of a special quality of uncertainty, but rather because the uncertainty information can be used to calculate the true statistic of interest, the probability that it will rain at all. If I use uncertainty to improve decision making, it is not *true* uncertainty, rather it is supplementary material that is required because the original information was not sufficient. In this sense uncertainty is no different from any other variable. If someone is asked to calculate a persons BMI and they are only given their height, of *course* their calculation will be improved if you provide their weight too.

We can actually prove this result is true mathematically. Let $A$ be a rational agent that needs to make a decision and let $U(p)$ be the utility function of agent $A$ that depends on the random variable, $p$ where $p$ is a probability. Additionally $p$ can be calculated using a density function $p=f(\mu, \sigma)$

The ma


The best "uncertainty visualisation" in decision making experiments is reliably the visualisation that makes the calculation of 

For these reasons decision making as a motivation for uncertainty visualisation will not be the focus of this review, as the uncertainty in this case is not "noise" and does not require any special visualisation.

### Exceedance Probability Map
For these reasons, if the statistic required to make the decision is visualised, there is no reason to include uncertainty on *that* graphic.



the uncertainty provided for this purpose is *not* the same as the uncertainty provided for transparency.
...
We will investigate several approaches for uncertainty visualisation in the following sections and in doing so, discuss how successful each approach is in fulfilling the signal-suppression criteria.
...
Some authors do not describe uncertainty as a means of transparency, but rather as a variable of importance in of itself [@Blenkinsop2000]. 
