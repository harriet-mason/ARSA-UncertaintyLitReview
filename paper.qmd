...
Therefore the best uncertainty visualisation is not am "uncertainty visualisation" at all, but simply the most accurate depiction of our raw data, that gives us a good idea of the limitations of our model.
...
If we want to make conclusions about how effective any uncertainty visualisation method is we need to look at the results of evaluation experiments. Unfortunately the illustrative methods we have used thus far, i.e. showing a graphic and saying "wow look at this", are lacking if we want any generalizable results. However, despite the abundance of uncertainty visualisation evaluation experiments, existing literature reviews have struggled to synthesise them into any common rules [@Kinkeldey2014; @Hullman2016]. Additionally, several authors have linked this problem to a  "task" or "goal" dependence in the evaluations.

Here we discuss the cause of this goal dependence, the current methods for evaluating uncertainty visualisation, and we ask if these methods are sufficient to evaluate graphics on their ability to perform singal-suppression.

## The Hermann grid illusion of visualisation experiments
When you look at a graphic, what statistics are you inferring from the visulisation? Depending on the graphic, you might extract several statistics at once. However, this is assuming I have just presented a graphic and said nothing. What is I asked you to extract the mean of $X$? Then it might be safe to assume you are doing inference on $\bar{X}$. 
...
@Kinkeldey2014 noted that most studies assessed users ability to extract the estimate and the uncertainty as separate signals, and expressed doubt that this was an effective evaluation tool for conveying their "single integrated value".
...
It is unclear if this sufficient to assess a visualisation on its ability to communicate a 
