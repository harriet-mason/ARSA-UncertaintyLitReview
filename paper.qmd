...
#| fig-subcap: 
#|   - "Mean with a 95% prediction interval"
#|   - "Picture 2"
#|   - "Picture 3"
#|   - "Picture 4"
#|   - "Picture 5"
...
# Generate data
set.seed(6)
ib_data <- tibble(x=c(rpois(500, 8)) 
ib_data %>%
  ggplot(aes(x=x)) +
...
p3 <- ib_data |>
  ...
  mutate(Probability = n / sum(n),
         texture_groups = ) |>
  ggplot(aes(x="", y=Probability)) +
  geom_bar_pattern(aes(pattern=binx, pattern_angle = binx, pattern_spacing = binx),
                   pattern = 'stripe',
                   fill    = 'white',
                   colour  = 'black',
...
p1b <- p1a %+% filter(state, variance_class=="highvar")
# Bivariate Map
...
p2b <- p2a %+% filter(state, variance_class=="highvar")
...
p3b <- p3a %+% filter(state, variance_class=="highvar")

# Maps I haven't been able to get working yet
...
pixel_1a <- state_a %>%
  as.data.frame() %>%
  select(my_id, temp, error) %>%
  read.uv(estimate="temp", error="error")

pixel_2a <- state_a %>% as("Spatial")

pix_a <- pixelate(pixel_2a, pixelSize = 3, id = "my_id")


pmap_a <- build_pmap(data = pixel_1a, distribution = "normal", pixelGeo = pix_a, id = "my_id", border = pixel_2a)
...
pmap_b <- build_pmap(data = pixel_1b, distribution = "normal", pixelGeo = pix_b, id = "my_id", border = pixel_2b)
