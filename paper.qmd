...
It would be impossible to discuss uncertainty visualisation without mentioning the concept of uncertainty communication and discussing how visualisation fits into that broader scope. Uncertainty visualisation is related to uncertainty communication in the same way that visualisation more broadly is related communication more broadly. Visualisation is fast, it can communicate more complicated ideas that what can be done with text alone, and 


*psychology of uncertainty communication*

It is difficult to read a paper that discusses uncertainty that does not make a nod towards uncertainty bias. In the case of uncertainty visualisation, these nods are not always relevant. Similarly to how contextual information is conflated with uncertainty when we try to define it, human attitudes to risk are often commented on in the discussions of uncertainty visualisation. Ironically, despite being mentioned in a large proportion of papers that discuss uncertainty risk aversion, has very to do with the psychology of uncertainty. As a matter of fact, it has so little to do with uncertainty communication that we would not even mention it in this paper were it not for the fact that ignoring it would suggest we had missed an important section of the literature.

Risk-aversion is the tendancy of people to prefer outcomes of low uncertainty to those outcomes with high uncertainty despute the outcomes with high uncertainty having a higher or equal expected outcome.

Uncertainty information is typically included in some form of statistical communication to offer some transparency to the viewer. Data analysis is a long and complicated process, so much so that even experts in the field can find themselves lost with a single incorrect assumption, so uncertainty can help pull back the curtain on this process and prevent people from understanding it as a human agnostic, entirely deterministic process. When science authors discuss "risk avoidance" in the uncertainty communication conversations, it holds with it some implication that heuristics such as risk avoidance are relevant in every conversation about uncertainty communication, however that is not true.

Risk and uncertainty are not perfect synonyms of one another. @Spiegelhalter2017 makes the distinction clear by specifying *risk* as *a precise random event* (such as a coin flip) while uncertainty is imprecision (such as the estimated probability associated with a coin that might be biased). Additionally, risk avoidance is observed when people have a choice between a *deterministic* outcome and a *risky* event that has higher expected value. How this psychological heuristic is therefore related to all uncertainty visualisation despite uncertainty communication being different in both the scenario (i.e. it is highly unlikely that we need to communicate a choice between a perfectly deterministic and an uncertain event) and in the type of uncertainty (since risk and uncertainty are not synonyms).
