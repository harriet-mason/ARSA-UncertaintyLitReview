...
Uncertainty visualisation is a uniquely difficult problem. @Hullman2016 commented that it is straightforward to show a value but it is much more complex to show uncertainty. There does not seem to be a consensus on *why* this is the case, but there are two main groups of thought that emerge when trying to justify the difficulty in uncertainty visualisation.

The first group believes uncertainty visualisation are unique due to the psychological heuristics involved in interpreting uncertainty [@Spiegelhalter2017; @Hullman2019]. While it is true that heuristics lead people to avoid uncertainty [@Spiegelhalter2017] there is no evidence that this psychological effect translates to a problem in visualisation. Were this the case, visualisations of death, poverty, and any other variable that elicits poor feelings would require their own field of visualisation. 

The second group sees uncertainty as a high dimensional visualisation problem as the difficulty is in working out how to add uncertainty into already existing graphics [@moritz2017trust; @Griethe2006]. If we view uncertainty visualisation as a high dimension problem we include uncertainty as we would any other variable and use high-dimension visualisation techniques to incorporate it. This is the approach taken in @fig-bivariate. 

The problem with the high dimension approach is that it treats uncertainty the same as we would any other variable. Typically we want visual channels to be separable, that is, the data represented in one visual channel does not interfere with the others [@Smart2019]. Unfortunately, this means that applying normal visualisation methods to "uncertainty visualisation" will prevent uncertainty and signal being read together as a "single integrated uncertain value" [@Kinkeldey2014], which is the goal of signal-suppression. Separating the uncertainty and the signal also allows for the uncertainty to be ignored, which is a common problem faced by most uncertainty visualisations [@uncertchap2022]. We can see this problem in @fig-bivariate, as it sends the message "this data has a spatial trend and the estimates have a large variance" rather than hiding the statistically invalid signal. 

If thses high-dimensional approaches were to work, the uncertainty and the estimates would need to be mapped to integrable channels. That is, the channel that uncertainty and the estimates are mapped to would need to be separately manipulable, but read as a single channel by the human brain. While most aesthetics *are* seperable, there are some variables that have been shown to be integrable, such as colour hue and brightness [@Vanderplas2020]. We can more effectively approach this method by also leveraging visual semiotics and mapping more uncertain values to lighter colors [@Maceachren2012].
...
@fig-bivariate2 shows a bivariate color map where uncertainty is mapped to color value, and temperature is mapped to color hue. It is clear the spatial trend is still visible in the high uncertainty case. This does not necessarily mean integrability will never be a useful tool for signal suppression, but without a strong understanding of how these visual channels collapse down into a single channel, it may not be a reliable option. Relying on integrability could create unintended consequences of uncertainty visualization, such as displaying phantom signals or hiding justified signals. Additionally, multi-dimensional colour palettes can make the graphics harder to read and hurt the accessibility of the plots [@Vanderplas2015]. A more viable option might be to warp the space uncertainty and signal are mapped to ourselves.
...
#| warning: false
#| label: fig-vsup
#| fig-cap: "A map made with a VSUP. The counties of Iowa are coloured acording to it's average daily temperature and the variance in temperature, although the space they have been mapped to is slightly warped."
#| fig-subcap: 
#|   - "High Variance Data"
#|   - "Low Variance Data"
#|   - "VSUP Palette"
