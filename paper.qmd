...
There are some papers that suggest a novel plot that does not serve to fix a specific visualisation issue. These papers suggest ways to include even *more* information in a visualisation, and shift the information overload from the visualisation author to the viewer. Clever methods of combining several types of information, such as VSUPs or tours, can better help us understand how our brains process information in a graphic, but visualisations that simply seek to display as much information as possible fail to understand that visualisations are primarily as a tool to summarise information. Poor understanding of our goals in generating a visualisation will not be remedied by overloading the viewer.  
...
- Perceptual Tasks: These experiments identify perceptual tasks that are/could be associated with uncertainty, and test its association with uncertainty, check it's number of distinguishable levels, and/or check how quickly/accurately people can draw the uncertainty information from the graphic. 
...
Each of these types of papers may have distinct goals in their visualisation, but their problems and conflicting results share a common source. Some of these issues come from issues deeper than the visualisation field (e.g. the lacking definitions of uncertainty) while some are specific to visualisation (e.g. the difficulty in identifying the signal of a graphic). Below we will go through the five key issues in uncertainty visualisation, describe how they mainfest in the uncertainty visualisation literature, and offer some tentative solutions to these problems.
...
- perceptual tasks need to know what you are uncertain about
- perceptual tasks don't consider

## Uncertainty Visualisation and the Grammar of Graphics
The first issue in uncertainty visualisation is there does not seem to be a clear rule for what makes a particular visualisation a depiction of uncertainty.
