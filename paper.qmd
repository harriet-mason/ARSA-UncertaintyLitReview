...
# Visualising uncertainty as signal
...
# Including uncertainty as an indepdendent variable
...
Uncertainty visualisation is a uniquely difficult problem in visualisation. @Hullman2016 commented that it is straightforward to show a value but it is much more complex to show uncertainty. There are two primary theories as to *why* uncertainty visualisation is unique. 

The first group believes uncertainty visualisation are unique due to the psychological heuristics involved in interpreting uncertainty [@Spiegelhalter2017; @Hullman2019]. While it is true that heuristics lead people to avoid uncertainty [@Spiegelhalter2017] there is no evidence that this psychological effect translates to a problem in visualisation. Were this the case, visualisations of death, poverty, and any other variable that elicits poor feelings would require its own field of visualisation. 

The second group sees uncertainty as a high dimensional visualisation problem as the difficulty is in working out how to add uncertainty into already existing graphics [@moritz2017trust; @Griethe2006]. If we view uncertainty visualisation as a high dimension problem we include uncertainty as we would any other variable, and allow for the signal to be interpreted independent of the variance. This prevents uncertainty and signal being read together as a "single integrated uncertain value" [@Kinkeldey2014] and allows for uncertainty to be ignored [@uncertchap2022]. Additionally, even when reading a map that has a more complicated colour palette than a choropleth map (such as a bivariate map) participants still read the map according to the same principles of the choropleth map [@Ndlovu2023]. This means that even when uncertainty is technically included in the graphic, the take away can still be invalid if the map is read incorrectly. The signal-suppression needs to be communicated through the "gist" of the graphic, as that is primary purpose of a visualisation [@Spiegelhalter2017].  The @fig-bivariate plot's "gist" is "this data has a spatial trend and the estimates have a large variance". It is clear the uncertainty and signal are being communicated separately rather that as a single uncertain value.

This highlights the key problem of viewing uncertainty visualisation as a simple high dimensional problem. While we are trying to include a new variable, uncertainty cannot be treated the same as any other variable. Typically we want visual channels to be separable, that is, the data represented in one visual channel does not interfere with the others, and most research in this area is for this goal [@Smart2019]. Uncertainty visualisation, however, should endeavor for integrability, that is, the channels interfere with one another to allow the uncertainty to suppress the signal. While most aesthetics *are* read separately, colour hue and brightness seem to be integrable [@Vanderplas2020] so even though they can be individually manipulated, they are processed by the brain as a single visual channel. Additionally color value has a more intuitive connection to uncertainty than hue [@Maceachren2012], but it should be mapped such that lighter colours are more uncertain than darker colours. If we could utilise integrability effectively, them the two dimensional space defined by colour value and colour hue could be mentally "flattened" into a single dimension of "statistically valid signal".

@fig-bivariate2 shows a bivariate colour map where uncertainty is mapped to colour value, and temperature is mapped to colour hue. It is clear the spatial trend is still visible in the high uncertainty case. This does not necessarily mean integrability will never be a useful tool for signal suppression, but without a strong understanding of how these visual channels collapse down into a single channel, it may not be a reliable option. Additionally, adding so many variations to colour in a map might make the graphics harder to read and suppress statistically significant signals, while also hurting the plots accessibility [@Vanderplas2015]. A more viable option might be to warp the space uncertainty and signal are mapped to ourselves.
...
trans_map <- my_map_data |>
  mutate(scaled_var = (variance - min(variance))/(max(variance)-min(variance)))
...
ptb <- pta %+% filter(trans_map, variance_class=="highvar")
```
...
by using a VSUP which collapses the colour space such that high uncertainty values cannot be extracted. It is unclear how useful VSUPs are at actually combining signal and noise and therefore suppressing plot level insights, as they have only been tested on simple value extraction tasks that require evaluating a single point [@Correll2018; @Ndlovu2023] rather than looking for spatial relationships (which is arguably what maps are for). Following along with this trend, the next way we might consider visualising uncertainty is to combine uncertainty and signal at the earlier stage so the "suppressed signal" is represented by a single variable. This statistic can then be expressed in a one dimensional colour space, which is a method adopted by the Bayesian surprise metric map [@Ndlovu2023] and the exceedance probability map [Lucchesi2017]. 

This problem has been noticed by others in the field, that typically use this method (specifically in the spatial uncertainty context) and a desire for representations that integrate uncertainty and signal is one of the reasons for the invention of the value-suppressing uncertainty pallet [@Correll2018].

Value-suppressing uncertainty pallets (VSUP) were developed as a method that would allow the signal and the noise to be interpreted together such that insights gained by the viewers of a plot are appropriately *suppressed* by the uncertainty. Hence the name of the pallet. depicted this map colouring approach and several other extensions on the typical choropleth map that differ in where in the visualisation process they combine the noise and signal information into a single concept of "valid signal".
