...
The second group sees uncertainty as a high dimensional visualisation problem as the difficulty is in working out how to add uncertainty into already existing graphics [@moritz2017trust; @Griethe2006]. If we view uncertainty visualisation as a high dimension problem we include uncertainty as we would any other variable, and allow for the signal to be interpreted independent of the variance. This prevents uncertainty and signal being read together as a "single integrated uncertain value" [@Kinkeldey2014] and allows for uncertainty to be ignored [@uncertchap2022]. Additioanlly, this integration needs to be communicated through the "gist" of the graphic, as that is primary purpose of a visualisation [@Spiegelhalter2017]. 
The @fig-bivariate plot's "gist" is "this data has a spatial trend and the estimates have a large variance". It is clear the uncertainty and signal are being communicated separately.

Uncertainty visualisation is difficult because we need the uncertainty axis to interfere with the signal axis. When we typically visualise two independent variables we *do not* want them to interfere with each other, however the opposite is true for uncertainty visualisation. For uncertainty to perform signal suppression, it must interfere with the visual channel that signal was mapped to. It needs to interfere enough that viewers do not see signals that are simply the product of noise.
